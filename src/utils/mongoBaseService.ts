import { InsertManyOptions, PipelineStage } from "mongoose";
...
export class MongooseBaseService<IMongooseModel extends Document> {
  ...
  async getAll(): Promise<IMongooseModel[]> {
    return (await this._mongooseModel.find({}).lean()) as IMongooseModel[];
  }
  ...
  async getDistinct({ distinct, query }: { distinct: string; query: IQuery }): Promise<any[]> {
  ...
  async createOrUpdate(query: any, data: any, options: any = {}) {
  ...
  async createMany(data: any, options: InsertManyOptions = {}): Promise<IMongooseModel[]> {
    return (await this._mongooseModel.insertMany(data, options)) as IMongooseModel[];
  }
  ...
  async populate(iPopulate: IPopulate): Promise<{
    ...
    const { query, sort, select, populate, page = 0, limit = 10 } = iPopulate;
    const [docs, count] = await Promise.all([
      this._mongooseModel
        .find(query)
        .populate(populate)
  ...
  async getOneAndPopulate(iPopulate: IPopulate): Promise<any> {
    const { query, populate } = iPopulate;
    return await this._mongooseModel.findOne(query).populate(populate);
  }

  async updateOneAndPopulate(iUpdateAndPopulate: IUpdateAndPopulate): Promise<any> {
